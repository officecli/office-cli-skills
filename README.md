...
When the hosted trial is used up, create or purchase a hosted key from [officecli.io/pricing](https://officecli.io/pricing),
then save it:
...
OfficeCLI can be used by AI agents through public skills when you want the agent to route Office tasks
into the local binary instead of calling the CLI manually. This is optional: the primary interface is
still the `officecli` command.
...
The OpenClaw installer will install `openclaw-officecli`, create `config.yaml` from the example when
needed, and try to auto-install the `officecli` binary when it is missing.
...
### Is this repository the complete OfficeCLI source code?

No. This repository contains public install docs, demos, scripts, skill definitions, and plugin wrappers.
It is the public GitHub SEO and onboarding surface for the OfficeCLI binary.
...
No. It distributes binary install docs, demos, scripts, and optional local skill wrappers.
