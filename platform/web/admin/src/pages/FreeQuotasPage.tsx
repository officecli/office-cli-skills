            ...
            {quotas.map((quota) => (
              <div key={quota.id} className="admin-card-muted p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button type="button" className="admin-secondary-button" onClick={() => update.mutate({ id: quota.id, free_limit: quota.daily_limit + 1 })}>
                    <ShieldPlus size={14} /> Add 1 trial generation
                  </button>
                </div>
                <div className="mt-4 text-xs text-outline">Usage date {quota.usage_date}</div>
                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  <div className="admin-code-card rounded-2xl p-4"><div className="info-eyebrow-tight text-outline">Limit</div><div className="mt-2 text-white">{formatNumber(quota.daily_limit)}</div></div>
                  <div className="admin-code-card rounded-2xl p-4"><div className="info-eyebrow-tight text-outline">Used</div><div className="mt-2 text-white">{formatNumber(quota.daily_used)}</div></div>
                  <div className="admin-code-card rounded-2xl p-4"><div className="info-eyebrow-tight text-outline">Remaining</div><div className="mt-2 text-white">{formatNumber(quota.remaining)}</div></div>
                </div>
