...
function renderAgentSkillsRoute(pathname: string) {
  ...
  if (normalizedPath === agentSkillsHubPath) {
    return compact([
      ...
      '      <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/8 text-primary text-xs font-headline uppercase tracking-[0.28em] mb-6 border border-primary/25">officecli-skills public repository</span>',
      ...
      '      <p class="text-lg md:text-xl text-outline-variant leading-relaxed max-w-4xl mt-6">officecli-skills is the public GitHub repository and install surface for Claude Code, Codex, OpenClaw, and other AI agents that need local PPTX, DOCX, XLSX, and workbook-backed report workflows.</p>',
      '    </section>',
  ...
  return compact([
    ...
    '      <p class="text-outline-variant text-sm leading-relaxed max-w-3xl mt-4">Related pages: officecli-skills overview, install, Claude Code, Codex, OpenClaw, and FAQ.</p>',
    '    </section>',
    '  </main>',
    '</div>',
  ])
...
export function renderRouteApp(pathname: string) {
  ...
  return compact([
    ...
    '          <h1 class="font-headline text-6xl md:text-8xl font-bold tracking-tighter leading-[0.9] mb-8 text-white">Generate <span class="text-primary italic">PPTX</span>, DOCX, XLSX, REPORT, and IMG Outputs From One AI CLI</h1>',
    '          <p class="text-xl text-outline-variant max-w-xl mb-10 leading-relaxed font-light">OfficeCLI is a local-first AI document generation CLI for terminal workflows. Generate PPTX, DOCX, XLSX, workbook-backed REPORT outputs, and standalone IMG visuals with your own LLM endpoint or the OfficeCLI image service, without a backend stack or cluster, then review decks and expand into broader document automation.</p>',
    '        </div>',
    '      </div>',
    '    </section>',
    ...
    '      <h2 class="font-headline text-4xl font-bold text-white mb-6">Generate PPTX, DOCX, XLSX, REPORT, and IMG Outputs</h2>',
    '      <p class="text-outline-variant text-lg mb-8 leading-relaxed">Stay in the terminal and run AI document automation from one local-first CLI. Install once, connect your LLM, and generate PPTX, DOCX, XLSX, REPORT, and standalone IMG outputs without standing up extra infrastructure.</p>',
    '    </section>',
