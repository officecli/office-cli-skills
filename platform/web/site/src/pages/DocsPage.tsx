    ...
    <main className="overflow-x-hidden pt-28 px-8 md:px-16 max-w-[1440px] mx-auto pb-24">
      <section className="max-w-3xl">
        ...
        <p className="text-outline-variant text-lg leading-relaxed">
          OfficeCLI is a local-first CLI for document operations. Today it is strongest at generation and PPT review. Platform workflows remain optional for paid access, hosted runtime, billing, and preview publishing.
        </p>
      </section>
      ...
      <section className="mt-16 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        <article className="bg-surface-low rounded-2xl border border-outline-variant/10 p-8">
          ...
          <p className="text-outline-variant leading-relaxed">Generate PPTX, DOCX, XLSX, and HTML outputs. Review and score PPTX decks. Keep files local by default, or publish online previews when configured.</p>
        </article>
        <article className="bg-surface-low rounded-2xl border border-outline-variant/10 p-8">
          <h2 className="font-headline text-2xl font-bold text-white mb-4">External vs hosted</h2>
          <p className="text-outline-variant leading-relaxed mb-6">External mode uses your own LLM endpoint for local workflows. Hosted mode is for platform-backed access, billing-aware usage, and managed runtime behavior.</p>
