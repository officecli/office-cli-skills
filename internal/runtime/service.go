...
func NormalizePPTXBackendWithWarnings(value string) (string, []engine.GenerateIssue, error) {
	...
	if value == "" {
		return PPTXBackendOfficegen, nil, nil
	}
	switch value {
	case PPTXBackendOfficegen:
...
func deprecatedPPTXBackendWarning() engine.GenerateIssue {
	return engine.GenerateIssue{
		...
		Field:   "pptx_backend",
		Message: "pptx backend artifact-experimental is no longer supported; use --pptx-backend officegen.",
	}
}
...
func BuildPPTXFromJSONWithOptions(ctx context.Context, llm engine.LLMClient, progress engine.ProgressEmitter, content, fallback, requestedStyle string, enableImages, localPreview bool, options PPTXBuildOptions) ([]byte, string, []engine.GenerateIssue, []byte, []byte, error) {
	_, backendWarnings, err := NormalizePPTXBackendWithWarnings(options.Backend)
	if err != nil {
...
func buildProjectPlanDeck(slides []officegen.Slide, deckTitle string) []officegen.Slide {
	...
	out := []officegen.Slide{
		cover,
		{
			Title:         "Decision Snapshot: GO",
			Layout:        "comparison",
			Variant:       "comparison-spotlight",
			NarrativeRole: "summary",
			SectionTitle:  "Readiness",
			Subtitle:      "Proceed now; GTM and Support stay on watch.",
			Sections: normalizeSections([]officegen.SlideSection{
				{Heading: "GO", Detail: "Scope frozen; notes drafted"},
				{Heading: "GTM Watch", Detail: "Owner review remains"},
				{Heading: "Support Watch", Detail: "Routing drill remains"},
			}, 3),
		},
		{
			Title:         "Gate Scorecard: Proceed",
			Layout:        "dashboard",
			Variant:       "kpi-band",
			NarrativeRole: "summary",
			SectionIndex:  1,
			SectionTitle:  "Readiness",
			Subtitle:      "All gates clear enough to launch.",
			Metrics: []officegen.MetricCard{
				{Label: "Scope", Value: "PASS", Note: "Frozen"},
				{Label: "GTM", Value: "WATCH", Note: "Owner review"},
				{Label: "Support", Value: "WATCH", Note: "Routing drill"},
		...
		{
			Title:         "Risk Controls: Covered",
			Layout:        "dashboard",
			Variant:       "kpi-band",
			NarrativeRole: "action",
			SectionIndex:  2,
			SectionTitle:  "Execution and Decisions",
			Subtitle:      "Approval assigns triggers; it does not ignore risk.",
			Metrics: []officegen.MetricCard{
				{Label: "Quality", Value: "TRIGGER", Note: "Defect spike"},
				{Label: "Support", Value: "TRIGGER", Note: "Routing miss"},
				{Label: "Rollback", Value: "READY", Note: "Owner named"},
			},
		},
		{
			Title:         "Decision Request: Approve GO",
			Layout:        "closing",
			Variant:       "closing-decision-banner",
			NarrativeRole: "closing",
			SectionIndex:  2,
			SectionTitle:  "Execution and Decisions",
			Subtitle:      "Approve launch with watched GTM and Support lanes.",
			Sections: normalizeSections([]officegen.SlideSection{
				{Heading: "Approve", Detail: "Proceed with watched lanes"},
				{Heading: "Hold Only If", Detail: "Critical blocker appears"},
				{Heading: "Owner", Detail: "Launch lead tracks daily"},
			}, 3),
		},
	}
