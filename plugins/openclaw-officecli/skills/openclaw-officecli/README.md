...
`openclaw-officecli` lets OpenClaw users generate local `pptx`, `docx`, `xlsx`, `report`, and standalone `img` files with natural language from existing Telegram, Discord, Slack, and similar channels.
...
- The agent should read top-level `image_generation` before standalone `img` requests; standalone images are server-provider controlled
...
- Say "generate a launch image" directly in Telegram / Discord / Slack
...
3. `officecli config set-generation` and `officecli config set-license` have been completed, or the skill is allowed to fill them in automatically; standalone `img` requires `set-license` and does not use local generation image settings
...
- `generate a landscape launch image about our collaboration platform`
